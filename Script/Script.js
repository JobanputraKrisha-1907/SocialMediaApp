...
$(document).ready(function () {
    $("#clicked").click(function () {
        ...
        if ($("body").hasClass("dark-mode")) {
            $("#white-color, #white-2color").css("filter", "brightness(5)");
        } else {
            $("#white-color, #white-2color").css("filter", "none");
        }
    });
});
...
// Handle the "Share" button click event to save the post and append it to UI
    $('#shareButton').click(function () {
        ...
        var fileInput = $('#postImageInput').get(0);
        ...
        // Create a data object with userId and caption
        ...
        success: function (response) {
            ...
            response.forEach(function (post) {
                ...
                var htmlBlock = '<div class="post-main">' +
                    ...
                    '<span><i id= heart class="like-icon fa-solid fa-heart" aria-hidden="true" data-post-id="' + post.PostID + '"></i></span>' +
                    ...
                    '<p class="title"><span>' + username + '</span> &nbsp;:&nbsp;' + post.Caption + '</p>' +
...
$(document).ready(function () {
    ...
    $('#postStoryBtn').on('click', function () {
        ...
        if (files.length === 0) {
            alert('Please select an image to post a story.');
            return;
        }
    ...
    function FetchLastStoredStory() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#story-section').prepend(storyHTML);
            },
            error: function (xhr, status, error) {
        ...
        $.ajax({
            ...
            success: function (response) {
                response.forEach(function (story) {
                    ...
                    // Append the story HTML block to the UI
                    $('#story-section').append(storyHTML);
                });
            },
            error: function (xhr, status, error) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(function (user) {
                    ...
                    $('#follow-section').append(userHTML);
                });
    ...
    // Call fetchUsersAndAppend() function on page load
    $(document).ready(function () {
        fetchUsersAndAppend();
    ...
    $(document).on('click', '.like-icon', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                FetchPostDataAndAppend2();
                $('.Like-Count[data-post-id="' + PostId + '"]').html('<span class="like-count" style="font-weight:bold;">' + data.likeCount + ' Likes</span>');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.forEach(function (comment) {
                ...
                var commentHtml = '<div class="follow-sectionn" data-comment-id="' + comment.CommentID + '">' +
                    '<div class="profile-follow profile-foolow-hovering">' +
                    '<div class="profile-follow-left">' +
                    '<div class="profile-follow-image">' +
                    ...
                    '</div>' +
                    '<div>' +
                    '<span><i id="heart" class="like-comment fa-regular fa-heart " aria-hidden="true" data-comment-id="' + comment.CommentID + '" data-post-id="' + PostId + '"></i></span>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
                ...
                if (comment.ParentCommentId === null) {
                    ...
                    var $parentComment = $container.find('.follow-sectionn[data-comment-id="' + comment.ParentCommentId + '"]');
...
$(document).on("click", ".Delete-comment-btn", function () {
    ...
    $(document).on("click", "#sendReplyBtn", function () {
        ...
        if (commentText !== "") { // Check if commentText is not empty
            ...
            alert('Comment text is empty or contains only spaces.');
        }
    });
...
function escapeHtml(unsafe) {
    ...
    //                $('.follow-sectionn[data-comment-id="' + commentId + '"]').addClass('liked');
    //            } else {
    //                $('.follow-sectionn[data-comment-id="' + commentId + '"]').removeClass('liked');
    //            }
    //        },
    //        error: function (xhr, status, error) {
        ...
        success: function (notifications) {
            ...
            notifications.forEach(function (notification) {
                ...
                if (notification.trim() !== "") {
                    $('.notification-list').prepend('<li class="notification" style="display: flex; align-items: center;">' + notification + '</li>');
                }
            });
        },
        error: function (xhr, status, error) {
